...
impl TablatureWidget {
    ...
    fn get_image_data(&mut self, width: usize, height: usize) -> &Vec<u8> {
...
impl Widget<AppData> for TablatureWidget {
    ...
    fn paint(&mut self, ctx: &mut PaintCtx, data: &AppData, env: &Env) {
        ...
        for bar in data.bars.iter().enumerate() {
            let x = bar.0 as f64;
        ...
        let layout = text
            ...
            .build()
            .unwrap();
        ctx.draw_text(&layout, (100.0 + timex, 25.0));

        // Let's burn some CPU to make a (partially transparent) image buffer
        let image_data = self.get_image_data(256, 256);
        let image = ctx
